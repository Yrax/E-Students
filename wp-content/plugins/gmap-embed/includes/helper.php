...
if (!function_exists('gmap_embed_no_of_post')) {
	...
	function gmap_embed_no_of_post()
	{
		...
		// Defensive: Validate object and property
		return (is_object($count_posts) && isset($count_posts->draft)) ? intval($count_posts->draft) : 0;
	}
}
...
if (!function_exists('_wgm_can_add_new_map')) {
	...
	{
		// Defensive: Sanitize option value
		return (gmap_embed_no_of_post() == 0 || sanitize_text_field(get_option('_wgm_is_p_v', 'N')) == 'Y');
	}
}
