...
trait AssetHandler
{
	...
	public function getLocalizedScripts()
	{
		...
		if (isset($_GET['tag']) && sanitize_text_field(wp_unslash($_GET['tag'])) == 'edit') {
			if (isset($_GET['wgm_map_create_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['wgm_map_create_nonce'])), 'wgm_create_map')) {
				...
				$current_map_marker_lat_lng = explode(',', sanitize_text_field(get_post_meta($map_id, 'wpgmap_latlng', true)));
				$current_map_marker_lat = isset($current_map_marker_lat_lng[0]) ? esc_html($current_map_marker_lat_lng[0]) : 40.73359922990751;
				$current_map_marker_lng = isset($current_map_marker_lat_lng[1]) ? esc_html($current_map_marker_lat_lng[1]) : -74.02791395625002;
				$wgm_object = [
	...
	private function registerCommonScripts()
	{
		...
		wp_register_script(
			'wp-gmap-api',
			esc_url_raw('https://maps.google.com/maps/api/js?key=' . rawurlencode($this->wpgmap_api_key) . '&libraries=places&language=' . rawurlencode($srm_gmap_lng) . '&region=' . rawurlencode($srm_gmap_region) . '&callback=Function.prototype'),
			array('jquery')
		);
	}
